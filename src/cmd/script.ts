namespace jaxon.cmd {
    export class script {
        ...
        */
        static includeScriptOnce(command) {
            command.fullName = 'includeScriptOnce';
            let fileName = command.data;
            // Check for existing script tag for this file.
            let oDoc = jaxon.config.baseDocument;
            let loadedScripts = oDoc.getElementsByTagName('script');
            let iLen = loadedScripts.length;
            for (let i = 0; i < iLen; ++i) {
                let script = loadedScripts[i];
                if (script.src) {
                    if (0 <= script.src.indexOf(fileName))
                        return true;
                }
            }
            return script.includeScript(command);
        }
        ...
        */
        static includeScript(command) {
            command.fullName = 'includeScript';
            let oDoc = jaxon.config.baseDocument;
            let objHead = oDoc.getElementsByTagName('head');
            let objScript = oDoc.createElement('script');
        ...
        */
        static removeScript(command) {
            command.fullName = 'removeScript';
            let fileName = command.data;
            let unload = command.unld;
            let oDoc = jaxon.config.baseDocument;
            let loadedScripts = oDoc.getElementsByTagName('script');
            let iLen = loadedScripts.length;
            for (let i = 0; i < iLen; ++i) {
        ...
        */
        static confirmCommands(command) {
            command.fullName = 'confirmCommands';
            let msg = command.data;
            let numberOfCommands = command.id;
        ...
        */
        static makeWrapper(origFun, args, codeBlocks, returnVariable, context) {
            let originalCall = '';

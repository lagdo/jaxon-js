    ...
    export class dom {
        ...
        */
        static $(sId) {
            ...
            let obj = oDoc.getElementById(sId);
            if (obj)
                return obj;
        ...
        */
        static getBrowserHTML(sValue) {
            let oDoc = jaxon.config.baseDocument;
            if (!oDoc.body)
                return '';

            let elWorkspace = jaxon.$('jaxon_temp_workspace');
            if (!elWorkspace) {
            ...
            let browserHTML = elWorkspace.innerHTML;
        ...
        */
        static willChange(element, attribute, newData) {
        ...
        */
        static findFunction (sFuncName) {
            let context = window;
            let namespaces = sFuncName.split(".");
            for(let i = 0; i < namespaces.length && context != undefined; i++) {

namespace jaxon.tools {
    export class form {
        ...
        */
        getValues(parent) {
            ...
            if (parent && parent.childNodes)
                this._getValues(aFormValues, parent.childNodes, submitDisabledElements, prefix);
        ...
        */
        _getValues(aFormValues, children, submitDisabledElements, prefix) {
            var iLen = children.length;
            for (var i = 0; i < iLen; ++i) {
                var child = children[i];
                if (('undefined' != typeof child.childNodes) && (child.type != 'select-one') && (child.type != 'select-multiple'))
                    this._getValues(aFormValues, child.childNodes, submitDisabledElements, prefix);
                this._getValue(aFormValues, child, submitDisabledElements, prefix);
            }
        }
